...
Start the development client with `bun run dev`. Install a native development client with `bun run android` or `bun run ios`. The root `bun run dev:client` starts this app and the server.
